    ...
    <div className="bg-slate-50 min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="flex items-center space-x-2 text-xs text-slate-500">
          ...
          <span>/</span>
          <Link href="/shop" className="hover:text-royal-700">Shop</Link>
          <span>/</span>
          <Link href={`/category/${product.categorySlug}`} className="hover:text-royal-700">
            {product.category}
          </Link>
          <span>/</span>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            >
              ...
              {discountPercent > 0 && (
                <span className="absolute top-4 left-4 bg-accent-red text-white text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-wider shadow-md">
                  -{discountPercent}% OFF
                </span>
              )}
            </div>
                ...
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${
                      selectedImageIndex === idx ? 'border-royal-600 scale-95' : 'border-slate-200'
                    }`}
                  >
          ...
          <div className="lg:col-span-6 flex flex-col justify-between space-y-6">
            <div>
              <div className="flex items-center justify-between gap-4 mb-2">
                <span className="text-xs font-bold text-royal-600 uppercase tracking-wider bg-royal-50 px-3 py-1 rounded-full">
                  {product.category}
                </span>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-baseline space-x-3 mb-6">
                <span className="text-3xl font-black text-royal-800">
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${
                            isSelected
                              ? 'border-royal-600 bg-royal-600 text-white shadow-sm'
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100">
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 bg-royal-600 hover:bg-royal-700 text-white font-extrabold py-4 px-6 rounded-2xl shadow-lg flex items-center justify-center space-x-2 text-xs uppercase tracking-wider transition-all"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-accent-red hover:bg-accent-red-hover text-white font-extrabold py-4 px-6 rounded-2xl shadow-lg flex items-center justify-center space-x-2 text-xs uppercase tracking-wider transition-all"
                >
                  <span>Buy Now</span>
                </button>
        ...
        {/* Detailed Tabs: Description / Specs / Delivery / Reviews */}
            ...
            {activeTab === 'delivery' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200/60 space-y-2">
                  <Truck className="w-6 h-6 text-royal-600" />
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-6">
                <div className="flex items-center space-x-4 p-4 bg-slate-50 rounded-2xl border border-slate-200 max-w-sm">
                  <span className="text-4xl font-black text-royal-800">{product.rating}</span>
                  <div>
                    <div className="flex text-amber-400">
                      {[...Array(5)].map((_, i) => (
